#My take on Jamil's coding a transformer from scratch
import torch
import torch.nn as nn
import math

#refer https://asaiyru99.github.io/asaiy/portfolio/transformer

class InputEmbeddings(nn.Module):
    """
    initializes an embedding table with vocab size (rows) and
    d_model as columns (no. of dimensions that the word is embedded with)
    """
...
class LayerNormalization(nn.Module):
    """
    normalizes the input x, which is performed after the attention layer (during add + norm)
    """
...
#Create a structure of zeroes that represent the sentence (seq_length) and accommodates
        #the d_model dimension (column)
...
class Encoder(nn.Module):
    """
    the encoder initializes layers (which is the list of residual
     connections from nn.ModuleList)
    and returns the values from the encoding block along with normalization.
